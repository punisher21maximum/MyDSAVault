def modifiedBinarySearch(arr, l, r, target):
    ...
    Approach:
    Method 1: Find the two subarrays, apply binarySearch on each.

    Method 2: Modified binarySearch
    When searching in left and right halves, check in the sorted half first,
    To check if left is sorted, a[l] < a[mid]
    To check if right is sorted, a[r] > a[mid]
    '''
    if l > r:
        return -1
