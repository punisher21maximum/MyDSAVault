...
class BT:
    ...
    def reverseTreePathGivenPath(self, path, level):
        ...
        for i in range(1, (level // 2) + 1):
            path[i].value, path[level - i + 1].value = (
                path[level - i + 1].value,
                path[i].value,
            )
    ...
    def getNodePath(self, node, givenNode, path={}, level=1):
        ...
        if node.left:
            if self.getNodePath(node.left, givenNode, path, level + 1):
                return
        ...
        if node.right:
            if self.getNodePath(node.right, givenNode, path, level + 1):
                return

    def pathToN(self, node, path=[]):
        if not node:
            return
        ...
        if node.left:
            self.pathToN(node.left, path)

        if node.right:
            self.pathToN(node.right, path)
